...
public class MDAG implements ICacheAble {
    ...
    public MDAG(File dataFile) throws IOException {
        ...
        while ((currentString = dataFileBufferedReader.readLine()) != null) {
            ...
            //If the _transition PATHS of the previousString needs to be examined for minimization or
    ...
    /**
     * Calculates the length of the the sub-PATHS in a _transition PATHS, that is used only by a given string.
     *
     * @param str a String corresponding to a _transition PATHS from sourceNode
     * @return an int denoting the size of the sub-PATHS in the _transition PATHS
    ...
    private int calculateSoleTransitionPathLength(String str) {
        ...
        //(we are processing the outgoing transitions of nodes inside str's _transition PATHS,
        //the outgoing transitions of the MDAGNode at the top of the stack are outside this PATHS)

        transitionPathNodeStack.trimToSize();

        //Process each node in transitionPathNodeStack, using each to determine whether the
        //_transition PATHS corresponding to str is only used by str.  This is true if and only if
        //each node in the _transition PATHS has a single outgoing _transition and is not an accept state.
    ...
    public void removeString(String str) {
        if (sourceNode != null) {
            //Split the _transition PATHS corresponding to str to ensure that
            //any other _transition paths sharing nodes with it are not affected
            splitTransitionPath(sourceNode, str);

            //Remove from equivalenceClassMDAGNodeHashMap, the entries of all the nodes in the _transition PATHS corresponding to str.
            removeTransitionPathRegisterEntries(str);

            //Get the last node in the _transition PATHS corresponding to str
            ...
            if (!strEndNode.hasTransitions()) {
                ...
                if (soleInternalTransitionPathLength == internalTransitionPathLength) {
                    ...
                    //Remove the sub-PATHS in str's _transition PATHS that is only used by str
     ...
     * that corresponds to the _transition PATHS that will be next up for minimization processing.
     * <p/>
     * The "minimization processing start index" is defined as the index in {@code prevStr} which starts the substring
     * corresponding to the _transition PATHS that doesn't have its right language extended by {@code currStr}. The _transition PATHS of
     ...
     * to the _transition PATHS next up for minimization processing
     */
    private int calculateMinimizationProcessingStartIndex(String prevStr, String currStr) {
        ...
        if (!currStr.startsWith(prevStr)) {
            ...
            //The _transition PATHS of the substring of prevStr from this point will need to be submitted for minimization processing.
            //The substring before this point, however, does not, since currStr will simply be extending the right languages of the 
            //nodes on its _transition PATHS.
            ...
            mpsIndex = -1;    //If the prevStr is a prefix of currStr, then currStr simply extends the right language of the _transition PATHS of prevStr.
     ...
     * _transition PATHS corresponding to a given String from a given node.
     *
     * @param originNode the MDAGNode from which the _transition PATHS corresponding to str starts from
     * @param str        a String corresponding to a _transition PATHS in the MDAG
     * @return a HashMap of Strings to Objects containing:
     * - an int denoting the length of the PATHS to the first confluence node in the _transition PATHS of interest
     * - the MDAGNode which is the first confluence node in the _transition PATHS of interest (or null if one does not exist)
     */
    private HashMap<String, Object> getTransitionPathFirstConfluenceNodeData(MDAGNode originNode, String str) {
        ...
        //to the transitoin PATHS to the confluence node, as well as the actual confluence node
     ...
     * Performs minimization processing on a _transition PATHS starting from a given node.
     * <p/>
     * This entails either replacing a node in the PATHS with one that has an equivalent right language/equivalence class
     ...
     *
     * @param originNode the MDAGNode that the _transition PATHS corresponding to str starts from
     * @param str        a String related to a _transition PATHS
     */
    private void replaceOrRegister(MDAGNode originNode, String str) {
        ...
        //this on the next char in order to further processing down the _transition PATHS corresponding to str
    ...
    /**
     * 给节点添加一个转移路径<br>
     * Adds a _transition PATHS starting from a specific node in the MDAG.
     *
     * @param originNode the MDAGNode which will serve as the start point of the to-be-created _transition PATHS
     * @param str        the String to be used to create a new _transition PATHS from {@code originNode}
     */
    private void addTransitionPath(MDAGNode originNode, String str) {
        if (!str.isEmpty()) {
            ...
            // a _transition PATHS corresponding to it from originNode
    ...
    /**
     * 从登记簿中移除路径对应的状态们<br>
     * Removes from equivalenceClassMDAGNodeHashmap the entries of all the nodes in a _transition PATHS.
     *
     * @param str a String corresponding to a _transition PATHS from sourceNode
     */
    ...
    /**
     * 从给点节点开始克隆一条路径<br>
     * Clones a _transition PATHS from a given node.
     *
     * @param pivotConfluenceNode         the MDAGNode that the cloning operation is to be based from
     * @param transitionStringToPivotNode a String which corresponds with a _transition PATHS from souceNode to {@code pivotConfluenceNode}
     * @param str                         a String which corresponds to the _transition PATHS from {@code pivotConfluenceNode} that is to be cloned
     */
    private void cloneTransitionPath(MDAGNode pivotConfluenceNode, String transitionStringToPivotNode, String str) {
        ...
        char lastTransitionLabelChar = '\0';                                //Will store the char which labels the _transition to lastTargetNode from its parent node in the prefixString's _transition PATHS

        //Loop backwards through the indices of str, using each as a boundary to create substrings of str of decreasing length
        //which will be used to _transition to, and duplicate the nodes in the _transition PATHS of str from pivotConfluenceNode.
        for (int i = str.length(); i >= 0; i--) {
            ...
            {
                //Clone pivotConfluenceNode in a way that reassigns the _transition of its parent node (in transitionStringToConfluenceNode's PATHS) to the clone.
    ...
    private void addStringInternal(String str) {
        ...
        //in the _transition PATHS from sourceNode corresponding to prefixString.
        ...
        //Remove the register entries of all the nodes in the prefixString _transition PATHS up to the first confluence node
        //(those past the confluence node will not need to be removed since they will be cloned and unaffected by the 
        //addition of suffixString). If there is no confluence node in prefixString, then remove the register entries in prefixString's entire _transition PATHS
        ...
        //If there is a confluence node in the prefix, we must duplicate the _transition PATHS
        //of the prefix starting from that node, before we add suffixString (to the duplicate PATHS).
        ...
        //Add the _transition based on suffixString to the end of the (possibly duplicated) _transition PATHS corresponding to prefixString
     ...
     * @param prefixString          the String corresponding to the currently traversed _transition PATHS
    ...
    public HashSet<String> getStringsStartingWith(String prefixStr) {
        ...
        if (sourceNode != null)      //if the MDAG hasn't been simplified
        {
            MDAGNode originNode = sourceNode.transition(prefixStr);  //attempt to _transition down the PATHS denoted by prefixStr

            if (originNode != null) //if there a _transition PATHS corresponding to prefixString (one or more stored Strings begin with prefixString)
            {
                if (originNode.isAcceptNode()) strHashSet.add(prefixStr);
                getStrings(strHashSet, SearchCondition.PREFIX_SEARCH_CONDITION, prefixStr, prefixStr, originNode.getOutgoingTransitions());   //retrieve all Strings that extend the _transition PATHS denoted by prefixStr
            }
        } else {
            SimpleMDAGNode originNode = SimpleMDAGNode.traverseMDAG(mdagDataArray, simplifiedSourceNode, prefixStr);      //attempt to _transition down the PATHS denoted by prefixStr

            if (originNode != null)      //if there a _transition PATHS corresponding to prefixString (one or more stored Strings begin with prefixStr)
            {
                if (originNode.isAcceptNode()) strHashSet.add(prefixStr);
                getStrings(strHashSet, SearchCondition.PREFIX_SEARCH_CONDITION, prefixStr, prefixStr, originNode);        //retrieve all Strings that extend the _transition PATHS denoted by prefixString
            }
        }

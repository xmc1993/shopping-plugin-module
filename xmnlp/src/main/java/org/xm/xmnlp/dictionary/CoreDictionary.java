...
public class CoreDictionary {
        ...
        public static Attribute create(String natureWithFrequency) {
            try {
                ...
                for (int i = 0; i < natureCount; ++i) {
                    attribute.nature[i] = LexiconUtil.convertStringToNature(param[2 * i], null);
                    attribute.frequency[i] = Integer.parseInt(param[1 + 2 * i]);
                    attribute.totalFrequency += attribute.frequency[i];
                }
                return attribute;
            } catch (Exception e) {
                logger.warning("使用字符串" + natureWithFrequency + "创建词条属性失败！" + TextUtil.exceptionToString(e));
                return null;
            }
        }
    ...
    public final static String path = Xmnlp.Config.CoreDictionaryPath;

...
public class CustomDictionary {
    ...
    public static boolean load(String path, Nature defaultNature, TreeMap<String, CoreDictionary.Attribute> map, LinkedHashSet<Nature> customNatureCollector) {
        try {
            ...
            while ((line = br.readLine()) != null) {
                ...
                if (Xmnlp.Config.Normalization) param[0] = CharTable.convert(param[0]); // 正规化
    ...
    public static boolean add(String word) {
        if (Xmnlp.Config.Normalization) word = CharTable.convert(word);
        if (contains(word)) return false;
    ...
    static boolean loadDat(String path) {
        try {
            ...
            logger.warning("读取失败，问题发生在" + TextUtil.exceptionToString(e));
            return false;
        }
        return true;
    }
    ...
    public static CoreDictionary.Attribute get(String key) {
        if (Xmnlp.Config.Normalization) key = CharTable.convert(key);

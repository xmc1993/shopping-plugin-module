...
import org.xm.xmnlp.dictionary.common.CommonSynonymDictionaryEx;
...
public class CoreSynonymDictionaryEx {
    ...
    static {
        try {
            dictionary = CommonSynonymDictionaryEx.create(new FileInputStream(Xmnlp.Config.CoreSynonymDictionaryDictionaryPath));
        } catch (Exception e) {
    ...
    public static List<Long[]> convert(List<Term> sentence, boolean withUndefinedItem) {
        ...
        for (Term term : sentence) {
            ...
            switch (firstChar) {
                case 'm': {
                    if (!TextUtil.isAllChinese(term.word)) continue;
                }
                break;
                case 'w': {
                    continue;
